...
def calculates_results_stats(results_dic):
    ...
    # Z number of images (len result_dic)
    n_images = len(results_dic)

    # A number of correct dog matches (idx 3&4 = 1)
    n_correct_dogs = 0
    for key, value in results_dic.items():
        if value[3] == 1 and value[4] == 1:
            n_correct_dogs += 1

    # B number of dog images (idx 3 = 1)
    n_dogs_img = 0
    for key, value in results_dic.items():
        if value[3] == 1:
            n_dogs_img += 1

    # C number of correct non- dog matches (idx 3&4 = 0)
    n_correct_not_dogs = 0
    for key, value in results_dic.items():
        if value[3] == 0 and value[4] == 0:
            n_correct_not_dogs += 1

    # D number of not dog images (idx 3 = 0)
    n_not_dogs = 0
    for key, value in results_dic.items():
        if value[3] == 0:
            n_not_dogs += 1

    # E number of correct breed matches (idx 2&3 = 1)
    n_correct_breed = 0
    for key, value in results_dic.items():
        if value[2] == 1 and value[3] == 1:
            n_correct_breed += 1

    # Y number of label matches (idx 2 = 1)
    n_label_match = 0
    for key, value in results_dic.items():
        if value[2] == 1:
            n_label_match += 1
    ...
    """
    # 1a A/B
    pct_correct_dog = (n_correct_dogs / n_dogs_img) * 100

    # 1 b C/D
    if n_not_dogs > 0:
        pct_correct_notdog = (n_correct_not_dogs / n_not_dogs) * 100
    else:
        pct_correct_notdog = 0
    ...
    """
     Add key and value->  results_stats_dic['key1'] = value1
     Value:     Keys:
    ...
    results_stats_dic['n_correct_notdogs'] = n_correct_not_dogs
    results_stats_dic['n_notdogs_img'] = n_not_dogs
    results_stats_dic['pct_correct_notdogs'] = pct_correct_notdog

    results_stats_dic['n_correct_breed'] = n_correct_breed
    results_stats_dic['pct_correct_breed'] = pct_correct_breed

    results_stats_dic['n_match'] = n_label_match
    results_stats_dic['n_images'] = n_images
    results_stats_dic['pct_match'] = pct_label_match
